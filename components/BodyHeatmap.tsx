...
interface BodyHeatmapProps {
    ...
    onMusclePress?: (muscle: string, setCount: number) => void;
    showLabels?: boolean;
}
...
// Professional SVG polygon data from react-body-highlighter (adapted for React Native)
interface ISVGModelData {
    muscle: string;
    svgPoints: string[];
}

// Complete front view muscle data with body outline
const anteriorData: ISVGModelData[] = [
    // Add head for complete body
    {
        muscle: 'head',
        svgPoints: [
    ...
    // Add hip/groin area for complete torso
    {
        muscle: 'abductors',
        svgPoints: [
...
// Complete back view muscle data
const posteriorData: ISVGModelData[] = [
    {
        muscle: 'head',
        svgPoints: [
...
// Generate heatmap color based on set count
const getHeatmapColor = (setCount: number, maxSets: number): string => {
    if (setCount === 0) return '#E5E7EB'; // Light gray for no activity

    // Calculate intensity (0 to 1)
    const intensity = Math.min(setCount / Math.max(maxSets, 1), 1);

    // Create blue heatmap: light blue → dark blue
    const red = Math.round(255 * (1 - intensity * 0.9));   // Reduce red as intensity increases
    const green = Math.round(255 * (1 - intensity * 0.7)); // Reduce green as intensity increases
    const blue = 255; // Keep blue at maximum

    return `rgb(${red}, ${green}, ${blue})`;
};

export default function BodyHeatmap({ muscleData, onMusclePress, showLabels = true }: BodyHeatmapProps) {
    // Calculate max sets for color scaling
    const maxSets = Math.max(...Object.values(muscleData), 1);
    const [selectedMuscle, setSelectedMuscle] = useState<MuscleInfo | null>(null);

    const handleMusclePress = (muscle: string, setCount: number) => {
        if (onMusclePress) {
            onMusclePress(muscle, setCount);
        } else {
            // Create muscle info for the info block
            setSelectedMuscle({
                muscle,
                setCount,
                exercises: [] // Will be populated by parent component if needed
            });
        }
    };

    const renderMuscleGroup = (muscleGroup: ISVGModelData, view: 'front' | 'back') => {
        const setCount = muscleData[muscleGroup.muscle] || 0;
        const color = getHeatmapColor(setCount, maxSets);

        return muscleGroup.svgPoints.map((points, index) => (
            <Polygon
                key={`${muscleGroup.muscle}-${view}-${index}`}
                points={points}
                fill={color}
                stroke="#D1D5DB"
                strokeWidth={0.5}
                opacity={setCount > 0 ? 0.8 : 0.4}
                onPress={() => handleMusclePress(muscleGroup.muscle, setCount)}
            />
        ));
    };
        ...
        <View style={styles.container}>
            {showLabels && (
                <View style={styles.headerContainer}>
                    <Text style={styles.title}>Muscle Activity Heatmap</Text>
                    <Text style={styles.subtitle}>Last 30 days • Tap muscles for details</Text>
                </View>
            )}
            ...
            <View style={styles.bodyContainer}>
                ...
                <View style={styles.bodyView}>
                    ...
                    <Svg width={140} height={280} viewBox="0 0 100 200" style={styles.bodySvg}>
                        {/* Body outline - front */}
                        <Polygon
                            points="50 0 45 0 40 5 38 15 42 25 45 30 48 40 30 45 20 50 15 60 10 75 8 90 6 105 8 120 12 135 18 150 25 165 30 180 35 195 40 200 60 200 65 195 70 180 75 165 82 150 88 135 92 120 94 105 92 90 85 75 80 60 70 50 52 40 55 30 58 25 62 15 60 5 55 0"
                            fill="none"
                            stroke={Colors.divider}
                            strokeWidth={1}
                            opacity={0.3}
                        />
                        {anteriorData.map(muscleGroup => renderMuscleGroup(muscleGroup, 'front'))}
                    </Svg>
                </View>
                ...
                <View style={styles.bodyView}>
                    ...
                    <Svg width={140} height={280} viewBox="0 0 100 200" style={styles.bodySvg}>
                        ...
                        <Polygon
                            ...
                            opacity={0.3}
                        />
                        {posteriorData.map(muscleGroup => renderMuscleGroup(muscleGroup, 'back'))}
                    </Svg>
                </View>
            </View>

            {/* Selected Muscle Info Block */}
            {selectedMuscle && (
                <View style={styles.muscleInfoContainer}>
                    <TouchableOpacity
                        style={styles.muscleInfoHeader}
                        onPress={() => setSelectedMuscle(null)}
                    >
                        <Text style={styles.muscleInfoTitle}>{selectedMuscle.muscle.toUpperCase()}</Text>
                        <Text style={styles.closeButton}>✕</Text>
                    </TouchableOpacity>
                    <View style={styles.muscleInfoContent}>
                        <Text style={styles.muscleInfoSets}>{selectedMuscle.setCount} sets</Text>
                        <Text style={styles.muscleInfoSubtext}>Last 30 days</Text>
                    </View>
                </View>
            )}
            ...
            {showLabels && (
                <View style={styles.legendContainer}>
                    ...
                    <View style={styles.legendScale}>
                        ...
                        <View style={styles.legendItem}>
                            <View style={[styles.legendColor, { backgroundColor: getHeatmapColor(maxSets * 0.3, maxSets) }]} />
                            <Text style={styles.legendText}>Low</Text>
                        </View>
                        <View style={styles.legendItem}>
                            <View style={[styles.legendColor, { backgroundColor: getHeatmapColor(maxSets * 0.6, maxSets) }]} />
                            <Text style={styles.legendText}>Medium</Text>
                        </View>
                        <View style={styles.legendItem}>
                            <View style={[styles.legendColor, { backgroundColor: getHeatmapColor(maxSets, maxSets) }]} />
...
const styles = StyleSheet.create({
    ...
    muscleInfoSets: {
        fontSize: 20,
        fontWeight: '700',
        color: '#3B82F6', // Blue color to match the heatmap
        marginBottom: 2,
    },
    muscleInfoSubtext: {
        fontSize: 12,
